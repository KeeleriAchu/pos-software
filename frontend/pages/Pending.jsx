...
function CollectModal({ payment, onClose, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onSave(payment, parseFloat(amount))
      onClose()
...
export default function Pending() {
  ...
  const handleSave = (payment, collectedAmount) => {
    load()
    setPaymentReceipt({ ...payment, collected_now: collectedAmount })
  }
  ...
  if (paymentReceipt) {
    ...
    const newPaid = p.amount_paid + p.collected_now
    const remaining = Math.max(0, p.amount_due - newPaid)
    const d = new Date().toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' })
      ...
      <div>
        ...
        <div className="page-body">
          <div className="thermal-wrapper">
            <div className="thermal-receipt">
              <div className="thermal-header">
                ...
                <p>123 Main Street, City<br/>Phone: 9876543210</p>
                <div className="thermal-divider" />
                <p style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span>Bill: {p.bills?.bill_number || 'N/A'}</span>
                  <span>{d}</span>
                </p>
                <p style={{ textAlign: 'left', marginTop: '4px' }}>Customer: {p.customers?.name}</p>
                <div className="thermal-divider" />
              </div>
              
              <div className="thermal-summary">
                <span>Total Bill Amount:</span>
                <span>Rs {p.amount_due.toFixed(2)}</span>
              </div>
              <div className="thermal-summary" style={{ fontSize: '11px', color: '#555' }}>
                <span>Previously Paid:</span>
                <span>Rs {p.amount_paid.toFixed(2)}</span>
              </div>
              
              <div className="thermal-divider" />
              
              <div className="thermal-summary thermal-summary-bold">
                <span>Collected Now:</span>
                <span>Rs {p.collected_now.toFixed(2)}</span>
              </div>
              ...
              <div className="thermal-summary">
                ...
                <span>{remaining === 0 ? 'NIL' : `Rs ${remaining.toFixed(2)}`}</span>
              </div>
              {remaining === 0 && (
          ...
          <div className="receipt-actions">
            <button className="btn btn-accent flex-1" onClick={() => setPaymentReceipt(null)}>← Back</button>
            ...
            <button className="btn flex-1" style={{ background: '#25D366', color: 'white' }} onClick={() => {
                ...
                const msg = `🧾 *Payment Receipt*\n\nHello ${p.customers?.name},\nWe have received your payment of ₹${p.collected_now.toLocaleString('en-IN')} towards Bill: ${p.bills?.bill_number || 'N/A'}.\n\nRemaining Balance: ${remaining === 0 ? 'Nil' : `₹${remaining.toLocaleString('en-IN')}`}\n\nThank you! 🙏`
                window.open(`https://wa.me/91${phone}?text=${encodeURIComponent(msg)}`, '_blank')
            }}>💬 WhatsApp</button>
          </div>
        </div>
  ...
  return (
    <div>
      ...
      {collectModal && <CollectModal payment={collectModal} onClose={() => setCollectModal(null)} onSave={handleSave} />}
    </div>
  )
}
