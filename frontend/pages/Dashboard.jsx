          ...
          <div className="card">
            ...
            {bills.length === 0 ? (
              ...
              <div className="table-wrap" style={{ border:'none', borderRadius:0 }}>
                <table>
                  <thead><tr><th>Bill #</th><th>Customer</th><th>Amount</th><th>Collected</th><th>Remaining</th><th>Status</th></tr></thead>
                  <tbody>
                    {bills.map(b => (
                      <tr key={b.id}>
                        ...
                        <td><strong style={{color:'var(--green)'}}>{fmt(b.paid_amount)}</strong></td>
                        <td><strong style={{color:'var(--orange)'}}>{fmt(b.pending_amount)}</strong></td>
                        <td>
                          <span className={`badge ${b.payment_status==='paid'?'badge-green':b.payment_status==='partial'?'badge-orange':'badge-red'}`}>
                            {b.payment_status === 'paid' ? 'Cleared' : (b.payment_status.charAt(0).toUpperCase() + b.payment_status.slice(1))}
                          </span>
                        </td>
                      </tr>
                    ))}
