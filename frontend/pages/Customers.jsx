                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:20 }}>
                  {history.bills.map(b => (
                    <div key={b.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 14px', background:'var(--surface)', borderRadius:10 }}>
                      ...
                      <span className={`badge ${b.payment_status==='paid'?'badge-green':b.payment_status==='partial'?'badge-orange':'badge-red'}`}>{b.payment_status === 'paid' ? 'Cleared' : (b.payment_status.charAt(0).toUpperCase() + b.payment_status.slice(1))}</span>
                    </div>
                  ))}
                </div>
              )}
